...
public class CustomSecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (path.startsWith("/api/auth") || path.startsWith("/auth/google")) {
        ...
        if (path.startsWith("/swagger-ui") ||
                ...
                path.startsWith("/api/games-prices") ||
                path.startsWith("/api/comments") ||
                path.startsWith("/api/reviews") ||
                path.startsWith("/api/orders") ||
                path.startsWith("/api/payment/submitOrder") ||
                path.startsWith("/api/payment/vnpay-payment") ||
                path.startsWith("/api/payment/**") ||
                // path.startsWith("/api/purchases") ||
                path.startsWith("/api/order-purchases")) {
            filterChain.doFilter(request,response);
            return;
        }
